...
test("renders nearby old Commons photos only in the enabled build", async ({ page }) => {
  ...
  await openPoi(page);
  await expect(page.locator(".poi-sheet__photos")).toBeVisible();
  await expect(page.locator(".poi-sheet__photo")).toHaveCount(1);
  await expect(page.locator(".poi-sheet__photo-year")).toHaveText("1900");
  await expect(page.locator(".poi-sheet__photos-credit")).toContainText("Wikimedia Commons");
  await expect(page.locator(".poi-sheet__photo")).toHaveAttribute("target", "_blank");
  await expect(page.locator(".poi-sheet__photo")).toHaveAttribute("rel", "noopener noreferrer");
  expect(commonsRequests).toHaveLength(2);
  assertNoUnstubbedRequests(page);
});

test("hides the best-effort Commons strip when the provider fails", async ({ page }) => {
  await stubUpstream(page, { commonsStatus: 500 });
...
test("keeps the strip hidden when Commons has no dated old files", async ({ page }) => {
  await stubUpstream(page, { commonsGeosearch: { batchcomplete: "", query: { geosearch: [] } } });
