...
- Pipeline: length gate (≤4096 else `invalid`) → trim/NFC → extract first URL-ish token AND keep
  ...
  2. Plain pair: `/(-?\d{1,3}(?:\.\d+)?)[,\s]+(-?\d{1,3}(?:\.\d+)?)/` on the full text with word
     boundaries; both in range after validation.
  3. Apple: host `maps.apple.com` → `ll` param (lat,lng); optional `q` → label via `label()`
     validator; `address`-only → `no-coords`.
  4. Google full: hosts `www.google.com|google.com|maps.google.com|www.google.co.jp` path
     contains `/maps` → try in order: `query` param → `q` param → path segment `@lat,lng[,Nz]`
     (zoom from `Nz` when int 2–21).
  5. Shortlink hosts `maps.app.goo.gl|goo.gl|g.co` → `{ ok:false, reason:'shortlink' }`.
...
- [ ] Table-driven suite ≥ 30 cases: every recognizer happy path; Google `@` with/without zoom;
      `q=place+name` (no coords → falls through to `no-coords`); shortlinks; `javascript:` URI;
