...
- `vitest.config.ts`: environment `jsdom` (map/UI code touches DOM types), globals off (explicit
  ...
  thresholds: statements/branches/functions/lines ≥ 80% for implemented pure modules. Initial
  `coverage.include` covers `src/util/**/*.ts` only, because issue 01 placeholders in `src/state`,
  `src/security`, `src/integrations`, and `src/providers` are not implemented yet. Later issues add
  their owned directories to the same gate when real code lands (UI/map excluded).
...
- Example: `tests/unit/util/geo.spec.ts` testing a real `src/util/geo.ts` implementation of
  `haversineMeters(a, b)`, `viewportDiagonalMeters(bbox)`, and
  `metersToPixelsAtLat(meters, latitude, zoom)` (needed by §7.1 and issue 13).
...
1. `haversineMeters`: standard haversine, Earth radius 6371008.8 m; `viewportDiagonalMeters`
   = haversine between bbox SW and NE corners. `metersToPixelsAtLat` uses the Web Mercator ground
   resolution formula with tile size 512 for MapLibre GL. All helpers are pure, typed, exported.
...
- [ ] `npm run test` passes with coverage report; thresholds active for `src/util/**/*.ts`, with the
      documented expansion rule for later implemented pure modules.
